#!/usr/bin/env python3
"""
V6: Floating tile with frosted glass dome - combines proven glass bell approach
with floating tile aesthetic and blue background for clean transparent output.
"""
...
OUTPUT_DIR = Path("public/diorama/v6-dome")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Style description - frosted dome on floating tile, blue background, no reflections
DOME_TILE_STYLE = """
A floating isometric earth platform (15x15cm square, rounded organic edges) with a frosted glass dome on top,
the dome is matte finish with NO reflections, NO shine, subtle small knob handle on top,
visible soil cross-section on platform sides: dark brown topsoil, reddish clay middle, dark bottom layer,
light tan raised rim around the platform top edge,
...
Inside the dome, on top of the platform:
- Rich brown soil with 4 fluffy green moss patches in the corners
- Blue pebble near top-left moss, orange pebble right side, yellow pebble bottom
- A single brown seed in the center with a {crack_progress} crack forming
- Plenty of empty space inside the dome above the seed
...
KEEP the same dome, platform, soil, moss patches in corners, pebbles (blue, orange, yellow).
...
KEEP the same dome, platform, soil, moss patches, pebbles.
...
- The plant takes up about 1/4 of the dome's vertical space
...
Day {day} - the dome is becoming a home.
"""
...
- Takes up about 1/3 of dome's vertical space
...
- Takes up about 2/3 of dome's vertical space
...
- About {height:.0f}cm tall - fills most of the dome
- Massive ancient trunk with beautiful texture
- Canopy touches top and sides of dome
...
def generate_image(day: int, use_reference: bool = True) -> Path:
    ...
    if use_reference and ref_day:
        ...
        if ref_path.exists():
            ...
            contents = [
                f"Use this image as reference for style, dome shape, platform, and existing elements. Evolve it to day {day}:\n\n" + prompt,
                ref_image
            ]
...
def main():
    ...
    print("=" * 60)
    print("V6: Frosted Dome on Floating Tile")
    print("=" * 60)
    print()
